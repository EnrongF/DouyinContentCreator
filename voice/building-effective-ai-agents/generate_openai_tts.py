...
SCENE_GRAPH = ROOT / "scene-graphs" / "building-effective-ai-agents.json"
VOICE_MANIFEST = VOICE_ROOT / "voice-manifest.json"
CHUNK_DIR = VOICE_ROOT / "chunks"
REMOTION_AUDIO_DIR = ROOT / "remotion" / "public" / "building-effective-ai-agents" / "audio"
...
英文术语不要夸张升调；如果英文不自然，就用中文意思顺滑读出。
Agent 读作智能体或自然英文 Agent，Workflow 读作工作流，Single Agent 读作单智能体，
Multi-Agent 读作多智能体，Harness 读作运行护栏。CRM 读作 C-R-M。
""".strip()
...
def main() -> None:
    ...
    for chunk in manifest["chunks"]:
        ...
        reuse = output.exists() and output.stat().st_size > 2048
        if not reuse:
        ...
        chunk["remotionAudioFile"] = f"building-effective-ai-agents/audio/{chunk_id}.mp3"
